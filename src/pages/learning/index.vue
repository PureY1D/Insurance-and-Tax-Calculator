    ...
    <view v-show="tab===0" class="body">
      ...
      <view v-show="sub===0">
        ...
        <view class="card">
          ...
          <view v-if="hasSocial" class="know"><text class="kt">公积金基数规则</text><text class="kc">根据《住房公积金管理条例》：\n• 公积金基数 = 职工上一年度月平均工资\n• 基数下限: {{cfg.social.fundBaseMin}} 元（最低工资）\n• 基数上限: {{cfg.social.fundBaseMax}} 元（社平工资3倍）\n• 与社保基数可能不同</text></view>
          <view v-if="hasSocial" class="know"><text class="kt">当前配置比例</text><text class="kc">单位：养老{{cfg.social.employerPension}}% 医疗{{cfg.social.employerMedical}}% 失业{{cfg.social.employerUnemployment}}% 工伤{{cfg.social.employerInjury}}%\n个人：养老{{cfg.social.employeePension}}% 医疗{{cfg.social.employeeMedical}}% 失业{{cfg.social.employeeUnemployment}}%</text></view>
        </view>
